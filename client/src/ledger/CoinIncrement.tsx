...
export const CoinIncrement = ({ detail, setDetail }: properties) => {
    const handleCoinChange = (increment: number, sign: "+" | "-", type: "g" | "s" | "c") => {
        const currentCoin = { ...detail.coin };

        const coinChange: CoinPurseState = {
            sign: "+",
            gold: 0,
            silver: 0,
            copper: 0,
    ...
    const renderIncrement = (increment: string, type: "g" | "s" | "c") => {
        ...
        const color = type === "c" ? "copper" : type === "s" ? "silver" : type === "g" ? "gold" : "primary";
        return (
            <Stack direction="column">
                ...
                >
                    <Typography style={{ color: color === "copper" ? "#cc6633" : color }}>{increment}</Typography>
                </div>
                <Button
                    onClick={() => handleCoinChange(amount, "-", type)}
                    sx={{
